...
{#each assets as asset, i (i)}
	<article
		...
		style="--card-color: {category.color}; transition-delay: {(i % cardsNumber) * 60}ms"
	>
		<div class="relative">
			<a
				href={asset.link}
			...
			{#if asset.tags}
				<ul
					class="flex gap-x-2.5 items-center absolute bottom-0 right-0 bg-[#28292d] before:rotate-[265deg] before:absolute flex-wrap justify-end before:w-3 before:h-3 before:-left-3 before:bottom-0 before:z-10 before:rounded-full before:shadow-[-0.5rem_0.5rem_#28292d] before:rounded-br w-fit rounded-tl-md px-2 py-0.5"
					title="Tags"
				>
		...
		<div class="p-5 flex flex-wrap justify-between items-center relative">
			...
			{#if asset.licence}
				<div class="group/licence relative">
					<a
						class="bg-blue-600 transition-all hover:scale-105 inline-block px-2 py-1 rounded-md text-[13px] tracking-wide"
						target="_blank"
						rel="noopener noreferrer"
